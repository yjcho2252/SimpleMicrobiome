...
mod_spieceasi_ui <- function(id) {
  ...
  tagList(
    ...
    sidebarLayout(
      ...
      mainPanel(
        h4("SpiecEasi Network"),
        tags$div(
          id = ns("spieceasi_tab_container"),
          style = "max-width: 100%;",
          tabsetPanel(
            id = ns("spieceasi_active_tab"),
          tabPanel(
            "All",
        ...
        uiOutput(ns("spieceasi_status_separator")),
        h5(icon("circle-info"), "SpiecEasi Status"),
        uiOutput(ns("spieceasi_status_box"))
      )
    )
  )
}
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      session$sendCustomMessage(
        ...
        list(id = session$ns("spieceasi_tab_container"), width = paste0(width_px, "px"))
      )
    })
    ...
    observeEvent(input$run_network_btn, {
      ...
      if (isTRUE(network_running())) {
        showNotification("SpiecEasi is already running. Please wait for completion.", type = "message", duration = 3)
    ...
    draw_comparison_network_plot <- function(edge_tbl, edge_palette) {
      ...
      ggplot2::ggplot() +
        ...
        ggplot2::ggtitle(paste0(input$tax_level, "-Level SpiecEasi Differential Network (Two-Group Comparison)"))
    }

    output$comparison_network_plot <- renderPlot({
      if (is.null(input$run_network_btn) || input$run_network_btn < 1) {
        draw_wait_message("Click 'Run SpiecEasi' to start analysis.")
        return(invisible(NULL))
      }
      if (isTRUE(network_running())) {
        draw_wait_message("SpiecEasi analysis is running. Please wait...")
        return(invisible(NULL))
      }
      req(network_result())
    ...
    draw_network_plot <- function(res, connected_only = FALSE) {
      ...
      p <- ggplot2::ggplot() +
        ...
        ggplot2::ggtitle(
          if (connected_only) {
            paste0("NetCoMi SpiecEasi ", input$tax_level, "-Level Network (Connected)")
          } else {
            paste0("NetCoMi SpiecEasi ", input$tax_level, "-Level Network (All)")
          }
        )
    ...
    output$spieceasi_legend_box <- renderUI({
      ...
      tags$div(
        ...
        tags$div(
          ...
          uiOutput(session$ns("spieceasi_figure_legend"))
        )
      )
    })

    output$spieceasi_status_box <- renderUI({
      req(input$plot_width)
      box_width <- if (is.null(input$plot_width) || !is.finite(input$plot_width)) 800 else input$plot_width
      tags$div(
    ...
    output$spieceasi_status_separator <- renderUI({
      req(input$plot_width)
      box_width <- if (is.null(input$plot_width) || !is.finite(input$plot_width)) 800 else input$plot_width
      tags$hr(
        style = paste(
    ...
    output$spieceasi_figure_legend <- renderUI({
      req(input$tax_level, input$analysis_mode)
      active_tab <- if (is.null(input$spieceasi_active_tab) || !nzchar(input$spieceasi_active_tab)) "Network Plot (All)" else input$spieceasi_active_tab
      title_text <- paste0("SpiecEasi ", active_tab)
